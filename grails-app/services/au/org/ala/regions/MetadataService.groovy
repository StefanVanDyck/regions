...
class MetadataService {
    ...
    /**
     * Get species for speciefied speciesList
     * @param regionFid
     * @param regionType
     * @param regionName
     * @param regionPid
     * @param speciesListDrUid
     * @param showHubData
     * @param from
     * @param to
     * @param pageIndex
     ...
     * @return
     */
    def getSpecies(String regionFid, String regionType, String regionName, String regionPid, String speciesListDrUid, Boolean showHubData, String from = null, String to = null, String pageIndex = '0', String filter = null) {
        String speciesListItemsUrl = "${SPECIES_LIST_URL}/ws/speciesListItems/" + speciesListDrUid

        def speciesResponse = getJSON(speciesListItemsUrl)

        def speciesLsidList = speciesResponse.collect {item -> item.lsid}

        def params = buildBiocacheSearchOccurrencesWsUrl(regionFid, regionType, regionName, regionPid, null, null, from, to, pageIndex, showHubData, filter)
        def response = getJSON(params)

        //species count - TODO fix this
        String url = "${BIOCACHE_SERVICE_URL}/occurrences/facets"
        def query = [
                facets: 'names_and_lsid',
                flimit: '0'
        ] + buildCommonDownloadRecordsParams(regionFid, regionType, regionName, regionPid, null, null, from, to, showHubData, filter)
        ...
        if (response instanceof Map && response?.error) {
            ...
            def records = response.facetResults[0]?.fieldResult.findAll {
                it.label.split('\\|').size() >= 2
            }.findAll{
                List info = Arrays.asList(it.label.split('\\|'))
                speciesLsidList.contains(info.get(1))
            }.collect { result ->
                List info = Arrays.asList(result.label.split('\\|'))
                [
                        name      : info.get(0),
            ...
            [
                    totalRecords: records.count.sum(),
                    speciesCount: records.size(),
                    records     : records
            ]
        }
